...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    ubo1LayoutBinding.binding = 1;
    ubo1LayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    ubo1LayoutBinding.descriptorCount = 1;
    ubo1LayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    ubo1LayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding ubo2LayoutBinding = {};
    ubo2LayoutBinding.binding = 2;
    ubo2LayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    ubo2LayoutBinding.descriptorCount = 1;
    ubo2LayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    ubo2LayoutBinding.pImmutableSamplers = nullptr;

    std::vector<VkDescriptorSetLayoutBinding> bindings = { uboLayoutBinding, ubo1LayoutBinding, ubo2LayoutBinding };
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetModels().size(); ++i) {
        VkDescriptorBufferInfo modelBufferInfo0 = {};
        modelBufferInfo0.buffer = scene->GetBlades()[i]->GetBladesBuffer();
        modelBufferInfo0.offset = 0;
        modelBufferInfo0.range = NUM_BLADES * sizeof(Blade);

        VkDescriptorBufferInfo modelBufferInfo1 = {};
        modelBufferInfo1.buffer = scene->GetBlades()[i]->GetCulledBladesBuffer();
        modelBufferInfo1.offset = 0;
        modelBufferInfo1.range = NUM_BLADES * sizeof(Blade);

        VkDescriptorBufferInfo modelBufferInfo2 = {};
        modelBufferInfo2.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
        modelBufferInfo2.offset = 0;
        modelBufferInfo2.range = sizeof(BladeDrawIndirect);
        ...
        descriptorWrites[3 * i + 0].pBufferInfo = &modelBufferInfo0;
        ...
        descriptorWrites[3 * i + 1].pBufferInfo = &modelBufferInfo1;
        ...
        descriptorWrites[3 * i + 2].pBufferInfo = &modelBufferInfo2;
