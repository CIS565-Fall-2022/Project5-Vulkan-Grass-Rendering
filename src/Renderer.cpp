...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // Models + Blades
        { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER , static_cast<uint32_t>(scene->GetModels().size() + scene->GetBlades().size()) },   // ??? Why models + blades for image sampler and uniform buffer of modeldescriptorset
        ...
        // Compute
        //{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size() * 3) },   // GetBlades.size() will return number of blades, and each compute descriptor contains 3 storage buffers
        
        // ??? Can we have VK_DESCRIPTOR_TYPE_STORAGE_BUFFER 3 times each with GetBlades().size instead of above implementation?
        { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size()) },   //  for Blades buffer
        { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size()) },   // for culled Blades buffer
        { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size()) },   // for num blades buffer
