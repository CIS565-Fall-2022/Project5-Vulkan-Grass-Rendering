...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    // and corresponds to a resource of the same binding number in the shader stages.

    // descriptor: which type of resource descriptors are used for this binding

    // descriptorCount: the number of descriptors contained in the binding

    // stageFlags: bitmask of VkShaderStageFlagBits specifying which pipeline shader stages can access a resource for this binding
    ...
    if (vkCreateDescriptorSetLayout(logicalDevice, &createInfo, nullptr, &computeDescriptorSetLayout) != VK_SUCCESS) {
...
void Renderer::CreateGrassDescriptorSets() {
    ...
    if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, grassDescriptorSets.data()) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate descriptor set in CreateGrassDescriptorSets");
    }
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        grassBufferInfo.buffer = blade->GetModelBuffer();
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, computeDescriptorSets.data()) != VK_SUCCESS) {
        throw std::runtime_error("Failed to allocate descriptor set in CreateComputeDescriptorSets");
    }
...
void Renderer::CreateComputePipeline() {
    ...
    if (vkCreatePipelineLayout(logicalDevice, &pipelineLayoutInfo, nullptr, &computePipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create pipeline layout");
        throw std::runtime_error("Failed to create pipeline layout in CreateComputePipeline");
