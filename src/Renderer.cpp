...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    // Describe the binding of the descriptor set layout
    VkDescriptorSetLayoutBinding storageAllLayoutBinding = {};
    storageAllLayoutBinding.binding = 0;
    storageAllLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    storageAllLayoutBinding.descriptorCount = 1;
    storageAllLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    storageAllLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding storageCulledLayoutBinding = {};
    storageCulledLayoutBinding.binding = 1;
    storageCulledLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    storageCulledLayoutBinding.descriptorCount = 1;
    storageCulledLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    storageCulledLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding storageMetaLayoutBinding = {};
    storageMetaLayoutBinding.binding = 2;
    storageMetaLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    storageMetaLayoutBinding.descriptorCount = 1;
    storageMetaLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    storageMetaLayoutBinding.pImmutableSamplers = nullptr;

    std::vector<VkDescriptorSetLayoutBinding> bindings = { storageAllLayoutBinding, storageCulledLayoutBinding, storageMetaLayoutBinding };
...
void Renderer::CreateGrassDescriptorSets() {
    ...
    // Describe the desciptor set
    //Can use model because its allocated for model + grass
    VkDescriptorSetLayout layouts[] = { modelDescriptorSetLayout };
    VkDescriptorSetAllocateInfo allocInfo = {};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool; //Update descip pool?
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        //Do something with models but not images
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        VkDescriptorBufferInfo allBufferInfo = {};
        allBufferInfo.buffer = scene->GetBlades()[i]->GetBladesBuffer();
        allBufferInfo.offset = 0;
        allBufferInfo.range = sizeof(Blade) * NUM_BLADES;
        ...
        metaBufferInfo.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
        metaBufferInfo.offset = 0;
        metaBufferInfo.range = sizeof(BladeDrawIndirect);

        // Bind storage resources to the descriptor
        ...
        descriptorWrites[3 * i + 0].pBufferInfo = &allBufferInfo;
        ...
        descriptorWrites[3 * i + 2].pBufferInfo = &metaBufferInfo;
...
void Renderer::RecordComputeCommandBuffer() {
    ...
    for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) 
    {
        ...
        vkCmdDispatch(computeCommandBuffer, ((NUM_BLADES - 1) / 32)+1, 1, 1);
    }
