...
Camera::Camera(Device* device, float aspectRatio) : device(device) {
    ...
    cameraBufferObject.eye = glm::vec3(0.0f, 4.0f, 7.0f);
	cameraBufferObject.viewMatrix = glm::lookAt(cameraBufferObject.eye, glm::vec3(0.0f, 5.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
...
void Camera::UpdateOrbit(float deltaX, float deltaY, float deltaZ) {
    ...
    cameraBufferObject.eye = finalTransform * glm::vec4(0, 0, 0, 1);
